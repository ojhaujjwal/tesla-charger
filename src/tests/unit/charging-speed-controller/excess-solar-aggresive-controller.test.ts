...
describe('ExcessSolarAggresiveController', () => {
  let mockDataAdapter: { 
    getGridExportValue: ReturnType<typeof vi.fn>, 
    getValues: ReturnType<typeof vi.fn> 
  };
  let controller: ExcessSolarAggresiveController;

  beforeEach(() => {
    // Create a mock data adapter that uses Promise.all
    mockDataAdapter = {
    ...
    controller = new ExcessSolarAggresiveController(mockDataAdapter as any, {
      bufferPower: 100
    });
  });
  ...
  describe('determineChargingSpeed', () => {
    it('should limit charging speed to 32A', async () => {
      ...
      mockDataAdapter.getValues.mockResolvedValue({ voltage: 230 });
